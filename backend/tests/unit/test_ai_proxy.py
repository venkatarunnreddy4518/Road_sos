...
async def test_ai_chat_ollama(client, monkeypatch):
    ...
    mock_response.json.return_value = {"message": {"content": "Ollama local response"}}
...
async def test_ai_chat_openai(client, monkeypatch):
    ...
    mock_response.json.return_value = {
        "choices": [{"message": {"content": "OpenAI cloud response"}}]
    }
...
async def test_ai_chat_gemini(client, monkeypatch):
    ...
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "candidates": [{"content": {"parts": [{"text": "Gemini cloud response"}]}}]
    }
...
async def test_ai_chat_anthropic(client, monkeypatch):
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"content": [{"text": "Claude cloud response"}]}
